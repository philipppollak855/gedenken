...
class FamilyLinkAdmin(ModelAdmin):
    list_display = ('deceased_user', 'relative_user', 'relationship', 'role', 'permission_level', 'status', 'is_validated_by_admin', 'access_count', 'created_at')
    list_filter = ('role', 'permission_level', 'status', 'is_validated_by_admin', 'created_at')
    ...
    readonly_fields = ('created_at', 'updated_at', 'validated_at', 'last_accessed', 'access_count', 'last_ip_address')
    fieldsets = (
        ('Grunddaten', {
            'fields': ('deceased_user', 'relative_user', 'relationship')
        }),
        ('Rolle & Berechtigungen', {
            'fields': ('role', 'permission_level', 'status')
        }),
        ('Validierung', {
            'fields': ('is_validated_by_admin', 'validated_by', 'validated_at')
        }),
        ('Zugriffsstatistiken', {
            'fields': ('access_count', 'last_accessed', 'last_ip_address'),
            'classes': ('collapse',)
        }),
        ('Metadaten', {
...
class FamilyLinkInline(admin.TabularInline):
    ...
    def get_queryset(self, request):
        """Custom QuerySet mit Datenbank-Kompatibilität"""
        try:
            ...
            if "column api_familylink.id does not exist" in str(e):
                ...
                with connection.cursor() as cursor:
                    ...
                    cursor.execute(get_consistent_familylink_sql("WHERE fl.deceased_user_id = %s"), [self.parent_obj.id])
                    
                    # Erstelle Mock-Objekte für die Inlines
                    class MockFamilyLink:
                        def __init__(self, row):
                            ...
                            # Mock User-Objekte
                            self.deceased_user = self.parent_obj
                            self.relative_user = type('User', (), {
                                'id': row[11],
                                'get_full_name': lambda: f"User {row[11]}"
                            })()
        ...
        try:
            ...
            if "column api_familylink.id does not exist" in str(e):
                ...
                with connection.cursor() as cursor:
                    ...
                    # Verwende zentrale Hilfsfunktion für konsistente SQL-Query
                    from .views import get_consistent_familylink_sql
                    cursor.execute(get_consistent_familylink_sql("WHERE fl.relative_user_id = %s"), [self.parent_obj.id])
                    ...
                    class MockFamilyLink:
                        def __init__(self, row):
                            ...
                            # Mock User-Objekte
                            self.relative_user = self.parent_obj
                            self.deceased_user = type('User', (), {
                                'id': row[10],
                                'get_full_name': lambda: f"User {row[10]}"
                            })()
        ...
        try:
            ...
            if "column api_familylink.id does not exist" in str(e):
                ...
                with connection.cursor() as cursor:
                    ...
                    rows = cursor.fetchall()
                    links = [MockFamilyLink(row) for row in rows]
            else:
                links = []
...
def family_link_management_view(request):
    """
    Komfortable FamilyLink-Verwaltung - KOMPLETT NEU GESCHRIEBEN
    """
    ...
    try:
        if request.method == 'POST':
            ...
            role = request.POST.get('role', 'family_member')
            permission_level = request.POST.get('permission_level', 'view_only')
            ...
            if not deceased_user_id or not relative_user_id:
                ...
                try:
                    ...
                    existing_link = FamilyLink.objects.filter(
        ...
        try:
            ...
            if "column api_familylink.id does not exist" in str(e):
                ...
                try:
                    ...
                    # Verwende direkte SQL-Query für alte Datenbank-Struktur
                    cursor.execute("""
                        SELECT fl.link_id as id, fl.relationship, 
                               CASE WHEN fl.is_main_contact THEN 'main_contact' ELSE 'family_member' END as role,
                               CASE 
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                if "column api_familylink.id does not exist" in str(e):
                    # Fallback: Verwende Raw SQL für Existenz-Prüfung
                    from django.db import connection
                    with connection.cursor() as cursor:
                        cursor.execute("""
